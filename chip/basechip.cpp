...
BaseChip::BaseChip(): f_vdd(false), f_vpp(false),
                      f_addr_bus(0), f_data_bus(0) {
  /* logfile */
  f_logfile = CreateLogFile();
  f_last_tick = usGetSystemTick();
}

BaseChip::BaseChip(const BaseChip& src): f_vdd(src.f_vdd), f_vpp(src.f_vpp),
      f_addr_bus(src.f_addr_bus), f_data_bus(src.f_data_bus) {
  f_logfile = CreateLogFile();
  f_last_tick = usGetSystemTick();
...
void BaseChip::SetSize(unsigned long size) {
  ...
  /* sets the chip size */
  if (f_memory_area.size() != size) { f_memory_area.resize(size); }
}
...
void BaseChip::Write(void){
  /* checks the params */
  if (f_addr_bus >= f_memory_area.size()) {
    WriteToLog("Error in Write(addr=%06.6lX,data=%04.4X)", f_addr_bus, f_data_bus);
    return;
  }
...
BaseParChip::BaseParChip(): BaseChip(), f_oe(false),
                            f_ce(false), f_we(false) {}
...
BaseSerChip::BaseSerChip(): BaseChip(), f_ser_cs(false),
                            f_ser_clk(false), f_ser_data(false) {}
