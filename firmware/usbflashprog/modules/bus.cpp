...
DataBusConfig::DataBusConfig(): dSinPin(0xFF), dClkPin(0xFF), dClrPin(0xFF),
        dRckPin(0xFF), dSoutPin(0xFF) {}
...
AddrBusConfig::AddrBusConfig(): aSinPin(0xFF), aClkPin(0xFF),
        aClrPin(0xFF), aRckPin(0xFF) {}
...
void DataBus::configure(const DataBusConfig& config) {
    config_ = config;
    outRegister_.configure(config.dSinPin, config.dClkPin,
                           config.dClrPin, config.dRckPin);
    outRegister_.outputEnable();
    inRegister_.configure(config.dClrPin, config.dClkPin,
...
bool DataBus::isValidConfig_() const {
    return (config_.dClkPin  != 0xFF &&
            ...
            config_.dRckPin  != 0xFF &&
            config_.dSinPin  != 0xFF &&
...
void AddrBus::configure(const AddrBusConfig& config) {
    config_ = config;
    outRegister_.configure(config.aSinPin, config.aClkPin,
...
bool AddrBus::isValidConfig_() const {
    return (config_.aClkPin  != 0xFF &&
            config_.aClrPin  != 0xFF &&
            config_.aRckPin  != 0xFF &&
            config_.aSinPin  != 0xFF);
}
