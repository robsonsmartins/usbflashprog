...
TEST_F(OpCodeTest, get_value) {
    ...
    EXPECT_NEAR(OpCode::getValueAsFloat(buf, sizeof(buf)), 0.0f, 0.1f);
    buf[1] = 0x33;  // 51
    buf[2] = 0x21;  // 33
    EXPECT_EQ(OpCode::getValueAsByte(buf, sizeof(buf)), 0x33);
    EXPECT_EQ(OpCode::getValueAsBool(buf, sizeof(buf)), true);
    EXPECT_NEAR(OpCode::getValueAsFloat(buf, sizeof(buf)), 51.33f, 0.1f);
    buf[0] = kCmdResponseOk;
    EXPECT_EQ(OpCode::getValueAsByte(buf, sizeof(buf)), 0x33);
    EXPECT_EQ(OpCode::getValueAsBool(buf, sizeof(buf)), true);
    EXPECT_NEAR(OpCode::getValueAsFloat(buf, sizeof(buf)), 51.33f, 0.1f);
...
TEST_F(OpCodeTest, set_value) {
    ...
    EXPECT_NEAR(buf[1], 51, 1);
    EXPECT_NEAR(buf[2], 33, 1);
}
