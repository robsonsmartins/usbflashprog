 ...
 * @file    pcb45.cpp
 * @brief   Implementation file (C++) for Sivava PCB 4.5 Programmer
 *          Emulation Library
 *
...
/* Programmer object */
static SivavaPCB45Prog* objProgrammer = NULL;

// ---------------------------------------------------------------------------
/* Sivava PCB 4.5 Ports Map */
...
/* Inverted mask of A8 Address */
#define ADDR_MASK_D1 0xFFFEFF
/* Inverted mask of A16 Address */
#define ADDR_MASK_D2 0xFEFFFF

/* Data Port */

/* Address Bus shift register clock */
#define ADDR_BUS_CLK 0x01
/* Address Bus shift register data (bit D0) */
#define ADDR_BUS_DATA_D0 0x02
#define ADDR_BUS_DATA ADDR_BUS_DATA_D0
/* Address Bus shift register data (bit D1) */
#define ADDR_BUS_DATA_D1 0x10
/* Address Bus shift register data (bit D2) */
#define ADDR_BUS_DATA_D2 0x20
...
/* Data Bus shift register data (bit D2) (inverted) */
#define DATA_BUS_SER_IN_D2 0x40
/* Data Bus shift register data (bit D1) (inverted) */
#define DATA_BUS_SER_IN_D1 0x20
/* Data Bus shift register data (bit D0) (inverted) */
#define DATA_BUS_SER_IN_D0 0x10
...
bool _Load(void) {
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, CHIPLIB_REGISTRY_KEY, 0, NULL, 0,
        ...
        if (RegQueryValueExA(hKey, CHIPLIB_REGISTRY_NAME, NULL, NULL,
            ...
            RegSetValueEx(hKey, CHIPLIB_REGISTRY_NAME, 0, REG_SZ,
        ...
        if (RegQueryValueExA(hKey, CHIPLIB_REGISTRY_CHIPSIZE, NULL, NULL,
                             reinterpret_cast<LPBYTE>(&dwChipSize),
                             &dwBufLen) != ERROR_SUCCESS) {
            /* if error, then creates the default value in registry */
            RegSetValueEx(hKey, CHIPLIB_REGISTRY_CHIPSIZE, 0, REG_DWORD,
                          reinterpret_cast<const BYTE*>(&dwChipSize), dwBufLen);
        }
        RegCloseKey(hKey);
    }
    /* check for load library */
    /* library is loaded, exit */
    if (sChipLibName == strLibName && currentChipSize == dwChipSize &&
        hChipLib) {
        return true;
    }
    /* unload if loaded */
    ...
    if (hChipLib) {
        SetSize =
            reinterpret_cast<PSetSize>(GetProcAddress(hChipLib, "SetSize"));
        ...
        SetAddrBus = reinterpret_cast<PSetAddrBus>(
            GetProcAddress(hChipLib, "SetAddrBus"));
        SetDataBus = reinterpret_cast<PSetDataBus>(
            GetProcAddress(hChipLib, "SetDataBus"));
        GetDataBus = reinterpret_cast<PGetDataBus>(
            GetProcAddress(hChipLib, "GetDataBus"));
        SerialSetCS = reinterpret_cast<PSerialSetCS>(
            GetProcAddress(hChipLib, "SerialSetCS"));
        SerialSetClk = reinterpret_cast<PSerialSetClk>(
            GetProcAddress(hChipLib, "SerialSetClk"));
        SerialSetData = reinterpret_cast<PSerialSetData>(
            GetProcAddress(hChipLib, "SerialSetData"));
        SerialGetData = reinterpret_cast<PSerialGetData>(
            GetProcAddress(hChipLib, "SerialGetData"));
    }
    /* if error, unload */
    if (SetSize == NULL || SetVDD == NULL || SetVPP == NULL || SetOE == NULL ||
        SetCE == NULL || SetWE == NULL || SetAddrBus == NULL ||
        SetDataBus == NULL || GetDataBus == NULL || SerialSetCS == NULL ||
        SerialSetClk == NULL || SerialSetData == NULL ||
        SerialGetData == NULL) {
        _Unload();
    }
    /* check for errors */
    ...
    try {
        objProgrammer = new SivavaPCB45Prog();
    } catch (std::bad_alloc&) {
        objProgrammer = NULL;
    }
    /* check for errors */
...
SivavaPCB45Prog::SivavaPCB45Prog() : BaseProg() {
    /* initialization */
...
void SivavaPCB45Prog::EmuDataPort(unsigned char data) {
    ...
    if (f_control_port & OE) {
        ...
        if (flag_new != flag_old) {
            ...
            if (flag_new) {
                SerialSetData((data & SERIAL_DIN) != 0);
            }
            /* Set Serial Clock */
    ...
    if ((f_control_port & OE) == 0) {
        ...
        if (flag_new != flag_old && flag_new) { /* ADDR_BUS_CLK Toggle On */
            ...
            f_addr_serial_sr <<= 1;
            /*---------------------------------------------------------------*/
            /* set address bits in ADDR_BUS_DATA_Dx */
            if (port_new & ADDR_BUS_DATA_D0) {
                f_addr_serial_sr |= ~ADDR_MASK_D0;
            } else {
                f_addr_serial_sr &= ADDR_MASK_D0;
            }
            /*---------------------------------------------------------------*/
            if (port_new & ADDR_BUS_DATA_D1) {
                f_addr_serial_sr |= ~ADDR_MASK_D1;
            } else {
                f_addr_serial_sr &= ADDR_MASK_D1;
            }
            /*---------------------------------------------------------------*/
            if (port_new & ADDR_BUS_DATA_D2) {
                f_addr_serial_sr |= ~ADDR_MASK_D2;
            } else {
                f_addr_serial_sr &= ADDR_MASK_D2;
            }
            /*---------------------------------------------------------------*/
    ...
    if (flag_new != flag_old && !flag_new /* inv */) {
        ...
        /*----------------------------------------------------------------------*/
        /* refresh status with data bits */
        if (f_data_serial_sr & 0x80 /*inv*/) {
            f_status_port &= ~DATA_BUS_SER_IN_D2;
        } else {
            f_status_port |= DATA_BUS_SER_IN_D2;
        }
        /*----------------------------------------------------------------------*/
        if (f_data_serial_sr & 0x40 /*inv*/) {
            f_status_port &= ~DATA_BUS_SER_IN_D1;
        } else {
            f_status_port |= DATA_BUS_SER_IN_D1;
        }
        /*----------------------------------------------------------------------*/
        if (f_data_serial_sr & 0x20 /*inv*/) {
            f_status_port &= ~DATA_BUS_SER_IN_D0;
        } else {
            f_status_port |= DATA_BUS_SER_IN_D0;
        }
        /*----------------------------------------------------------------------*/
