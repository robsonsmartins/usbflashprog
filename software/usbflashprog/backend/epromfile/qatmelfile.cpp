    ...
    do {
        ...
        if (arec.address - address + 2 > result.size()) {
            result.append(QByteArray(
                static_cast<int>(arec.address - address) + 2 - result.size(),
                0xFF));
        }
        for (int i = 0; i < 2; i++) {
            result[i + arec.address - static_cast<quint32>(address)] =
                (arec.data >> (8 * i)) & 0xFF;
        }
...
bool QAtmelFile::write(QEpromFileType type, const QString &filename,
    ...
    while (total) {
        ...
        word <<= 8;
        word |= *(p + address);
        if (New_AtmelGenericRecord(address, word, &arec) != ATMEL_GENERIC_OK) {
            break;
        }
        if (Write_AtmelGenericRecord(&arec, fd) != ATMEL_GENERIC_OK) {
            break;
        }
        total -= len;
        address += len;
