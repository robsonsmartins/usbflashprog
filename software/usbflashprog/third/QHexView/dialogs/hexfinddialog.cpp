...
void HexFindDialog::updateFindOptions(int)
{
    ...
    static const QList<QPair<QString, unsigned int>> FLOAT_TYPES = {
        qMakePair<QString, unsigned int>(tr("float"),  QHexFindOptions::Float),
        qMakePair<QString, unsigned int>(tr("double"), QHexFindOptions::Double)
    };

    QGroupBox* gboptions = this->findChild<QGroupBox*>(HexFindDialog::GBOPTIONS);
    qDeleteAll(gboptions->findChildren<QWidget*>(QString(), Qt::FindDirectChildrenOnly));
    ...
    switch(mode)
    {
        case QHexFindMode::Text: {
            ...
            auto* cbcasesensitive = new QCheckBox(tr("Case sensitive"), gboptions);
        ...
        case QHexFindMode::Int: {
            ...
            auto* cbbits = new QComboBox(gboptions);
            ...
            auto* cbendian = new QComboBox(gboptions);
            ...
            gl->addWidget(new QLabel(tr("Type:"), gboptions), 0, 0, Qt::AlignRight);
            gl->addWidget(cbbits, 0, 1);
            gl->addWidget(new QLabel(tr("Endian:"), gboptions), 1, 0, Qt::AlignRight);
        ...
        case QHexFindMode::Float: {
            ...
            for(const auto& ft : FLOAT_TYPES) {
                auto* rb = new QRadioButton(ft.first, gboptions);
                rb->setChecked(first);
                vl->addWidget(rb);
                first = false;
            }
